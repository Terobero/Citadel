...
public class Robot extends IterativeRobot {
	...
	public void robotInit() {
		...
		autoChooser.addObject("Left Scale", 2);
		
		autoChooser.addDefault("Middle", 10);
		
		autoChooser.addObject("Right Duz", 20);
		autoChooser.addObject("Right Switch", 21);
		autoChooser.addObject("Right Scale", 22);
	...
	public void autonomousPeriodic() {
		...
		case 0: //left
		case 2: //right
			if (timerAutonomous.get() < 3.0)
				myRobot.driveCartesian(1.0, 0, 0);
			else
		...
		case 1: //middle
			...
			if (timerAutonomous.get() < 2) {
				myRobot.driveCartesian(0.5, 0, 0);
			}
			
			else if (timerAutonomous.get() < 4 && gameData.charAt(0) == 'L') {
				myRobot.driveCartesian(0, -0.5, 0);
			}
			
			else if(timerAutonomous.get() < 4 && gameData.charAt(0) == 'R') {
				myRobot.driveCartesian(0, 0.5, 0);
			}
			
			else if (centerXs.length < 1) { //kucuk esittir belki?
				if (stupid < 25)
					myRobot.driveCartesian(0, 0, -0.5);
				else if(stupid < 50)
					myRobot.driveCartesian(0, 0, 0.5);
				else
					stupid = 0;
				...
				if (currentArea >= 1000)
					...
					myRobot.driveCartesian(0.7, 0, 0);
			}
